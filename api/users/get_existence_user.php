    ...
    if (mysqli_num_rows($getHw) > 0) {
        if (mysqli_num_rows($get) > 0) {
            if ($data[0]['user_status'] == "active") {
                ...
                $queryGS = "SELECT `r`.`record_access_in`, `r`.`record_access_out`, `r`.`record_access_duration`, `h`.`hw_id`, `h`.`hw_name` 
                ...
                if (mysqli_num_rows($getGS) > 0){
                    ...
                    if ($rDur == "0" && $hwCode != $rHwId) {
                        ...
                        addLogSystem($dateTime, $data[0]['user_name'], $dataHw[0]['hw_name'], "Access Duplicate ($rHwNm)");
            ...
            set_response(false, "Unregistered Card", "NONE");
            addLogSystem($dateTime, $tag, $dataHw[0]['hw_name'], "Unregistered Card");
        }
    } else {
        set_response(false, "Unregistered Hardware", "NONE");
    }
} else {
